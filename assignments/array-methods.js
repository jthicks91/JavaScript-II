...
// Problem 1
//Figure out if all the runners had to pay a New York tax rate for their donations.

let taxes = [];

let newYorkTax = 1.8725;

runners.map(runner => {
  taxes.push(Math.round(newYorkTax * runner.donation * 100) / 100);
});

console.log(taxes);

// Problem 2
// Derive which runner(s) donated over $150.

const vipRunners = runners.filter(function(runner) {
...
// Problem 3
// Derive which runner(s) donated under $50.

const regRunners = runners.filter(function(runner) {
  return runner.donation < 50;
});

console.log(regRunners);
